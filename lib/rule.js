...
class Rule {
  ...
  constructor(config = {}) {
    ...
    internal.prefixes        = util.convertToArray(prefixes).map(s => new RegExp(`^${util.escapeRegExp(s)}`));
    internal.suffixes        = util.convertToArray(suffixes).map(s => new RegExp(`${util.escapeRegExp(s)}$`));
    internal.exceptPrefixes  = util.convertToArray(exceptPrefixes).map(s => new RegExp(`^${util.escapeRegExp(s)}`));
    internal.exceptSuffixes  = util.convertToArray(exceptSuffixes).map(s => new RegExp(`${util.escapeRegExp(s)}$`));
